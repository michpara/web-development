...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    subject(:game_guess_not_solution) { described_class.new(5, '4')}
    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(game_guess_not_solution).to_not be_game_over
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        second_letter = 'b'
        valid_input = '7'
        allow(game_loop).to receive(:player_input).and_return(first_letter, second_letter, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_four) {described_class.new(5, '5', 5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_four.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
end
