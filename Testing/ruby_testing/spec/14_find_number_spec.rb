...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    subject(:game_two) { described_class.new(5, 9, number_guessing) }

    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        guess = game_two.make_guess
        expect(guess).to eq(7)
      end
    end

    subject(:game_three) { described_class.new(8, 9, number_guessing) }
    ...
    subject(:game_four) { described_class.new(0, 3, number_guessing) }

    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        guess = game_four.make_guess
        expect(guess).to eq(1)
      end
    end

    subject(:game_five) { described_class.new(3, 3, number_guessing) }
  ...
  describe '#game_over?' do
    ...
    let(:number_over) { double('random_number', value: 8) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over) { described_class.new(0, 9, number_over, 8) }
    ...
    subject(:not_game_over) { described_class.new(0, 9, number_over, 7) }

    context 'when guess and random_number are not equal' do
      it 'is not game over' do
        expect(not_game_over).to_not be_game_over
      end
    end
  end
